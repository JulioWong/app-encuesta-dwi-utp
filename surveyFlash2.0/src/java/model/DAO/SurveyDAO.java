...
public class SurveyDAO implements SurveyInterface{
    ...
    public Survey get(int surveyId) {
        ...
        try {
            CallableStatement proc = DBconnection.prepareCall("{CALL spGetSurvey(?)}");  
            proc.setInt(1, surveyId);
            proc.execute();    
            final ResultSet rs = proc.getResultSet();  

            while (rs.next()) {
                sur.setSurveyId(rs.getInt("surveyId"));
                sur.setLogo(rs.getString("logo"));
                sur.setDescription(rs.getString("description"));
                sur.setCountQuestion(rs.getInt("countQuestion"));
                sur.setDiscount(rs.getString("discount"));
                sur.setBannerDiscount(rs.getString("bannerDiscount"));
                sur.setDiscountExpiration(rs.getString("timestampExpiration"));
            }
        } catch (SQLException ex) {
            Logger.getLogger(SurveyDAO.class.getName()).log(Level.SEVERE, null, ex);
        }    
